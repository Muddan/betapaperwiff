...
from ..model.collection.StoryCollection import StoryCollection
...
class StoryClass:
    ...
    def __init__(self):
        db = get_db()
        self.storyCollection = StoryCollection(collection=db['stories'])
        self.tagsCollection = db['tags']
        self.userCollection = db['users']

    # Story CRUD METHODS
    def publishStory(self, userId, tags, storyTitle, content, language, datePublished, headerImage, visibility='public', views=1, copyright=False):
    ...
    def getAllStories(self, pageNo=1):
        return self.storyCollection.getAllStories()
    ...
    def getStoryDetailsByStoryId(self, storyId):
        self.storyCollection.find_one_and_update({
            "storyId": storyId
        }, {
            "$inc": {"views": 1}
        })
    ...
    def storyLike(self, storyId):
        story = self.storyCollection.find_one_and_update(
            {"storyId": storyId}, {"$inc": {"likes": 1}})
        if story is None:
            return {
